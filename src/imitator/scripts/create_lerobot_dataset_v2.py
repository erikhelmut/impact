    ...
    for hdf5_file_path_str in hdf5_paths:
        ...
        try:
            with h5py.File(hdf5_file_path, "r") as f:
                ...
                # rescale feats_fdz to 24x32x1 array
                if feats_fdz.ndim == 2 and feats_fdz.shape[1] == 24 * 32:
                    feats_fdz = feats_fdz.reshape(-1, 24, 32)
                elif feats_fdz.ndim != 3 or feats_fdz.shape[1] != 24 or feats_fdz.shape[2] != 32:
                    raise ValueError(f"Unexpected feats_fdz shape: {feats_fdz.shape}. Expected (N, 24, 32) or (N, 24*32).")

                # create image from force distribution
                # 1. scale feats_fdz to [0, 255] range
                clim_z = (-0.2, 0.0)
                feats_fdz_scaled = (feats_fdz - clim_z[0]) * (255 / (clim_z[1] - clim_z[0]))
                feats_fdz_scaled = np.clip(feats_fdz_scaled, 0, 255)

                # 2. reshape to (N, 96, 96) for single channel
                resized_feats_fdz = np.array([
                    cv2.resize(frame, (96, 96), interpolation=cv2.INTER_AREA)
                    for frame in feats_fdz_scaled
                ], dtype=np.uint8)

                # add a channel dimension for single-channel images
                # currently image is size (N, 96, 96), we need (N, 96, 96, 3)
                # stack the same channel three times to make it RGB-like
                resized_feats_fdz = np.stack([resized_feats_fdz] * 3, axis=-1)

                # subsubsample the data to reduce size
                subsample = 4
                feats_fz           = feats_fz[::subsample]
                resized_feats_fdz  = resized_feats_fdz[::subsample]
                ...
                T = min(feats_fz.shape[0], resized_feats_fdz.shape[0], aruco_dist.shape[0], resized_images_np.shape[0], optitrack_trans_x.shape[0],
                ...
                force_distribution_data = resized_feats_fdz[:num_transitions]
                ...
                for i in range(num_transitions):
                    frame_for_lerobot = {
                        ...
                        "observation.image.feats_fdz": force_distribution_data[i],
