...
def calibrate(
    ...
    for w in waypoints:
        ...
        # Estimate the transforms using the recorded positions
        trans, quat = core.get_transformation()
        print ("current transformation: ", trans, quat)
...
def calibrate_ee_offset(result, transformation):
    ...
    ee_pose_robot_frame = Transformation.from_pos_quat(panda_ee_pos, panda_ee_ori)
    marker_pose_ot_frame = Transformation.from_pos_quat(ot_rb_pos, ot_rb_ori)
    ot_pose_robot_frame = Transformation.from_matrix(transformation)

    marker_pose_ee_frame = (ee_pose_robot_frame.inv * ot_pose_robot_frame * marker_pose_ot_frame).inv

    return marker_pose_ee_frame.translation, marker_pose_ee_frame.quaternion
    ...
    R_OT_RB_PANDA_EE = R_OT_PANDA_RB.inv() * Rotation.from_quat(panda_ee_ori)
...
def main():
    ...
    mean = np.median(transformation, axis=0)
